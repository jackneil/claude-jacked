...
- **All interfaces**: binds `0.0.0.0`. Anyone on any network the machine joins can reach the dashboard. You'll be asked to confirm you understand before this applies.
...
# {"current":"0.100.0","latest":"0.100.0","outdated":false,...}
...
| `/remember` | **Remember** - Routes a durable fact into the jacked Memory Vault as a typed, cross-repo note, falling back to native Claude Code memory when the vault is off |
...
| Version | Changes |
|---------|---------|
| **0.105.0** | **Account switching works from a remote browser, and the auto-updater stops rolling back healthy upgrades.** Three fixes found from one report ("switching sticks on Switching... from my other computer, and the GUI update never upgrades"). The dashboard minted its switch ids with `crypto.randomUUID()`, which only exists in a secure context, so over plain http on a hostname the click threw before any request and the button stayed on "Switching..." for the life of the page; ids now come from a `getRandomValues` fallback, minted inside the try, and the button always recovers. Credential switching was loopback-only even though the remote access dialogs promised it: it is now allowed for a remote client exactly when remote access is on and the client's address is inside the enabled scope (Tailscale ranges, or any address for all interfaces); the remote access settings themselves can only be changed from the host, every denial is reason-specific (in the toast and in a disabled Use Account button with a hint), remote decisions are logged and remote switches are attributed in Swap History, remote switching is rate limited, uvicorn's proxy trust is pinned so a tailscale serve viewer keeps its real address, and a startup warning describes any exposure beyond loopback. The updater's readiness probe tried to bind 127.0.0.1 with SO_REUSEADDR, which succeeds next to a 0.0.0.0 listener on macOS, so with remote access on it declared every healthy new service dead, rolled back, and declared the rollback dead too; it now probes with a TCP connect, identifies the port holder through /api/health, verifies that the expected build is the one answering, and names the build it saw in the log. CI is green again on ubuntu, macOS, and Windows: the service-status tests no longer depend on the host supervisor, a POSIX liveness probe can no longer turn into a console Ctrl+C broadcast on Windows, and the Linux-only os-release tests skip elsewhere. |
| **0.104.0** | **The Codex review engine defaults to GPT-6 Astra, and knows when the installed CLI cannot serve it.** OpenAI shipped GPT-6 Astra to Codex (CLI 0.153.0 or newer) and GPT-5.6 Sol, Terra, and Luna before it; jacked still defaulted to `gpt-5.6-luna`, the cheapest of the three, and had no idea what the installed CLI could run. The default model is now `gpt-6-astra` at `xhigh`, verified live on this account. `jacked dcr engine` reports the installed Codex CLI version, and when the configured model needs a newer CLI it reviews on `gpt-5.6-sol` and names the fix (`codex update`) instead of letting every reviewer job die on a 400. A stored `gpt-5.6-luna` (the default through 0.103.0) migrates to `gpt-6-astra` at read time with a reason that says how to keep Luna; an explicit Terra or Sol stays as set. The `ultra` effort level (Sol, Terra, Astra) is accepted everywhere effort is validated, with dashboard copy that says it adds task delegation a read-only reviewer does not need. Analytics pricing carries explicit rows for Claude Fable 5.1, Opus 5, and Mythos 5.1 (the family inference already priced them right; the rows make it visible). |
| **0.103.0** | **Review loops now converge instead of running until someone pulls the plug.** Field data from one branch: 12 review rounds, about 200 agents, 42M tokens, nine full test runs and roughly 30 hours for 6 real bugs, because every rule in the stack said "keep going" and none said when to stop. `/dcr` now states the branch's scope up front, every finding carries `introduced_by_branch`, and the fix phase sorts by it: a defect the branch introduced is fixed in the PR, always; a pre-existing one found next to the branch is fixed in the PR only when critical or a one-line change, and otherwise filed as an issue with the reviewer's evidence and linked from the PR. The loop stops when a wave yields no branch-introduced CRITICAL/MEDIUM and no critical pre-existing one; the default cap drops to 2 waves, a LARGE-tier third wave is reserved for an open CRITICAL, and a third wave that still finds branch-introduced defects means split the PR, never a fourth. "The answer is always yes" is gone from `/dcr` and `/dc`. The chain-of-command dispatch shape carries the same convergence and scoping rules for every fan-out (exhaustive on the first pass, convergent after; the full-suite gate is per push, not per fix batch), and the autonomous-run backstop treats a review loop as the one loop that "new work surfaced" never keeps alive. The Codex reviewer schema gains the `introduced_by_branch` field. A user's CLAUDE.md that says otherwise still wins. |
| **0.102.1** | **A slow shutdown no longer turns a healthy upgrade into a rollback.** `launchctl kickstart -k` and `bootout` block while launchd waits for the old service to drain, and a tray mid-shutdown takes over 12 seconds on a loaded machine; every launchctl call shared a flat 15-second timeout, so the first real 0.102.0 upgrade reported the restart handoff as refused and rolled back a build that was already coming up. Lifecycle calls now get a timeout sized to the handoff budget, and a timed-out restart checks whether the exact new generation became ready before it refuses. A healthy handoff still takes about 2 seconds. |
| **0.102.0** | **Every fan-out now follows the same risk-tiered budget, not just `/dcr`.** The chain-of-command skill, which loads into every session, carries a Dispatch shape section that binds the Agent tool, hand-written Workflow scripts, `/swarm`, and agent teams alike: classify the change first, then at most 4, 8, or 16 agents for a SMALL, MEDIUM, or LARGE change; findings are verified in the main loop (at LARGE, one strengthen-or-refute verifier per cluster, never one per finding); two review waves at most, the second over the fix delta only; a wave in which any agent died is incomplete, never clean, and resumes from the run journal; effort set on every agent call; "ultracode" authorizes the Workflow tool and nothing more; review stages run on the Codex engine when it is usable. Fan-out is sized to the remaining 5-hour usage window before it starts, one combined-lens reviewer per artifact is continued into the fix instead of a fresh agent re-reading everything, one party runs the browser gate per round, and research defaults to 4 lanes by 5 searches. `/dcr` validates findings in the main loop and runs the full suite once, on a frozen tree, as the final gate. `/retry` resumes a dead workflow from its journal first. The `/whats-next`, `/goal-maker`, and `/bhag` briefs review via `/dcr` tiers and never carry unbounded fan-out into a `/goal` pointer. Field data behind it: 38 agents for a 260-line module review, 28 for a small fix, about 100 for an 8-page build, and three waves where dead agents counted as clean. |
| **0.101.1** | **`jacked upgrade` keeps the tray under launchd, and a re-auth clears the refresh breaker.** The first real 0.101.0 upgrade on a Mac left the launchd job unloaded and a manual tray in its place. `launchctl bootout` returns at once while the job name stays registered until the old process finishes draining, and a `bootstrap` inside that window fails with EIO; the rollback hit the same error. launchd transitions now wait until the name is gone before they bootstrap, retry an EIO once the name is free, and roll back only after the same wait. `jacked service restart` re-arms an owned supervisor definition when no owned instance is left to hand off to, instead of spawning a manual tray that lives only until the next login. A Claude Code re-auth now clears the `invalid_grant` breaker a consumed refresh token left behind, so the authority guard imports later rotations and the heal sweep stops churning on a healthy account. |
...
| **0.87.0** | **`feat(statusline)`: a live Claude Code statusline on every session, installed and managed by jacked (on by default).** One line at the bottom of each session: model and reasoning effort (plus a fast-mode badge), context-window pressure with real token counts ("ctx 63% (633k/1.0M)"), both rate-limit windows with their reset times ("5h 7%->14:00 | 7d 88%->Sat 02:37"), and the account the session runs on (email · organization · plan tier), with percentages colored green / yellow at 60% / red at 85%. The renderer is a stdlib-only Python module that renders in about 20ms and is registered with the absolute path of jacked's own interpreter ("&lt;python&gt;" -m jacked.statusline), so it works identically on macOS, Linux, and Windows: no bash, no jq, and the spawn never resolves a python/python3/py name (the classic cross-machine failure; every `jacked install` re-resolves the path, so upgrades self-heal). The install is safe by design: `jacked install` never replaces a statusline jacked does not own (it keeps yours and says how to adopt its own), an explicit enable saves your previous statusline verbatim and disable/uninstall restore it (even when jacked's entry was removed out of band, but never over a newer one you chose since), an explicit disable is durable across upgrades (three-state store, the skill-packs doctrine), and a corrupt settings.json refuses the write instead of clobbering. Surfaces: default-on in `jacked install`, `--no-statusline`, `jacked statusline enable\|disable\|status`, and a dashboard toggle (Settings > Features > Statusline) with takeover/restore toasts. 157 new tests, plus a fix for two existing test files whose `jacked install` runs silently rewrote the real `~/.claude/jacked-guardrails` templates during every test run. |
