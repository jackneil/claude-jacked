...
- **Stack**: Python (FastAPI, uvicorn, hatchling, pystray/rumps); SQLite via `jacked/web/database.py`; integrates the Anthropic API, Qdrant (optional session search), and OAuth/credential flows
...
Reviewers focus on: missing edge cases in the design, over-engineering, architectural soundness, plan completeness. Repo emphasis: any auth/account/hook design must be checked for multi-account credential isolation and against the gatekeeper permission model.
...
- Does this respect the security gatekeeper / permission model — no path or command that silently bypasses `jacked/data/hooks/security_gatekeeper.py`?
