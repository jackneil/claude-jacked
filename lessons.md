...
- [1x] For any non-trivial implementation: brainstorm → write plan (writing-plans skill) → review plan → execute (subagent-driven-development) → double-check review → capture gaps → write plans for gaps → review → execute → repeat until clean. NEVER jump straight to coding. Always use the subagent-driven approach with parallel agents where possible. Do not stop to ask permission between iterations.
