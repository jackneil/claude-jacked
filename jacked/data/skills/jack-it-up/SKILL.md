...
**Why quality goes UP, not down:** review signal comes from lens focus, deterministic diagnostics, and per-finding validation — `/dcr` scales its reviewer fan-out to the RISK TIER of the change (one consolidated reviewer for a small mechanical change; the full persona/pre-mortem fan-out for large or security-sensitive work), and `/dc` skips its pre-mortem analyst for small contained changes. Fable-grade judgment still gates everything: the main loop adjudicates every finding before it triggers a fix, reads the diffs before trusting them, and makes every ship/fix/re-run call. Focused finders, stronger judge, tokens spent where a miss is expensive.
