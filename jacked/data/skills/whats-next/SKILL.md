---
name: whats-next
description: Use when the user wants help choosing or prioritizing what to work on next, or asks where to start. Weighs a coverage-matrix read plus plans, issues, commits, and lifecycle to decide the single highest-leverage initiative and forge a ready-to-run /goal brief.
---
...
2. If `.claude/commands/whats-next.md` exists (Glob) → read and follow it instead (legacy `/jacked-setup` output).
