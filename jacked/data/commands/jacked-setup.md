...
**Legacy dual-format detection.** An older `/jacked-setup` wrote two files per target, including a
`.claude/commands/<target>.md`. If a file exists at that path, VERIFY provenance before touching it
— a filename collision is not proof jacked wrote it:
```bash
grep -q 'Generated by /jacked-setup' .claude/commands/<target>.md 2>/dev/null && echo "JACKED_LEGACY" || echo "HAND_AUTHORED"
```
- **`JACKED_LEGACY`** → this target must be MIGRATED, not just regenerated:
  1. Read the legacy file. If it has a `## Repo Config` block, preserve it (it is the config of
     record — it may carry hand-edits the thin skill stub does not). If it has none, say so and
     fall back to full Step 2/3 discovery for this target.
  2. Write the new single file at `.claude/skills/<target>/SKILL.md` (Step 5 structure).
  3. DELETE the legacy `.claude/commands/<target>.md` — and TELL the user before asking consent:
     "this will also delete the old `.claude/commands/<target>.md` (it's jacked's previous
     generated copy; git history keeps it)."
  4. Say so in the Step 6 announce — including whether a `## Repo Config` was carried over or
     rebuilt from discovery.
- **`HAND_AUTHORED`** → do NOT delete or modify it. Tell the user: "`.claude/commands/<target>.md`
  doesn't look like `/jacked-setup` output — leaving it alone. Note: the generated skill at
  `.claude/skills/<target>/SKILL.md` takes precedence over a same-name command in Claude Code, so
  your command file will be shadowed. Delete or rename it yourself if that's not what you want."
  Then continue generating the skill normally.
...
- **If it is `OLD_FORMAT`, or a `JACKED_LEGACY` `.claude/commands/<target>.md` exists**: Warn — "⚠️ Your existing `/<target>` is the old format that depends on jacked being installed on every developer's machine. Regenerating makes it a single standalone skill." Ask: "Regenerate now?"
...
```
Migrated from the old dual-file format — deleted the `JACKED_LEGACY` `.claude/commands/<target>.md` (say whether its `## Repo Config` was carried over or rebuilt from discovery). Commit the deletion.
```
...
- Each target generates ONE file: `.claude/skills/<target>/SKILL.md`. When a `JACKED_LEGACY` `.claude/commands/<target>.md` exists (provenance verified per Step 4), carry its `## Repo Config` over and delete it. Never delete a `HAND_AUTHORED` file.
- The parts YOU author (frontmatter, header comment, `## Repo Config`, staleness check) must NOT contain the global repo-dispatch preamble, Glob fallback checks, or `~/.claude/` references. The engine body below the marker is copied byte-for-byte and stays untouched — its own internal `~/.claude/lenses` lookups and similar are part of the engine and already degrade gracefully; never "fix" them.
