...
5. **Spawn teammates with self-contained prompts.** Use the Task/Agent tool with `subagent_type: "general-purpose"` (they need write access). **Teammates do NOT inherit your conversation history** — they only load `CLAUDE.md`, MCP servers, and skills, plus the spawn prompt. So embed everything they need into each prompt:
   ...
   - **Per-teammate model:** assign cheaper models (e.g. Sonnet/Haiku) to mechanical, well-specified, isolated tasks and reserve Opus for the hard ones (tricky logic, debugging, architecture). This is the main cost lever. **Exception:** if the project/user `CLAUDE.md` mandates a single tier (e.g. an always-Opus policy), honor that and spawn every teammate on it.
