...
> **Note:** If `.claude/commands/whats-next.md` exists in the current repo, Claude Code runs it first. If that file is a config overlay (generated by `/jacked-setup`), it will delegate back to this global command with pre-discovered context. This global command handles both direct runs and overlay-delegated runs.
