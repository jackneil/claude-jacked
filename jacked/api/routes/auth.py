...
async def refresh_usage(account_id: int, request: Request):
    ...
    usage_data = await fetch_usage(account_id, db, access_token=effective_token, force=True)
...
async def refresh_all_usage(request: Request, skip_account: Optional[int] = Query(None)):
    ...
    async with _bulk_refresh_lock:
        ...
        for i, acct in enumerate(accounts):
            ...
            usage_data = await fetch_usage(acct["id"], db, access_token=effective_token, force=True)
